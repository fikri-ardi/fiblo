@extends('layouts.main', ['title' => 'Fiblo | Ubah Profil'])
        ...
        <form action="/profiles/{{ $user->slug }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="text-center">
                ...
                <div class="relative flex justify-center">
                    @if ($user->photo)
                    <img src="/storage/{{ $user->photo }}" class="img-preview rounded-circle w-48 h-48 object-cover mb-3 border-5 border-slate-200"
                        alt="{{ $user->name }}">
                    @else
                    ...
                    @endif
                    <input type="file" name="photo" id="photo" class="img-input absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40"
                        style="opacity: 0" onchange="previewImage()">
                </div>
            </div>
